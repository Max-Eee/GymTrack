...
class BackupService extends StateNotifier<BackupState> {
  ...
  Future<Directory> _getBackupDir() async {
    // Save to Downloads/GymTrack so it's visible in file managers
    final downloadsDir = Directory('/storage/emulated/0/Download/GymTrack');
    if (await downloadsDir.exists() || await _tryCreate(downloadsDir)) {
      return downloadsDir;
    }
    // Fallback: try getDownloadsDirectory (available on some platforms)
    final dlDir = await getDownloadsDirectory();
    if (dlDir != null) {
      final backupDir = Directory(p.join(dlDir.path, 'GymTrack'));
      if (await backupDir.exists() || await _tryCreate(backupDir)) {
        return backupDir;
      }
    }
    // Last resort: app-internal directory
